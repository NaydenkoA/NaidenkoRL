...
class agentDLVEnv(gym.Env):
    def __init__(
            ...
            spread = 0.005,
            impact_coeff = 0.0005,
            primordial_action_penalty = 0.3,
            penalty_damping = 0.1,
            action_penalty_scale = 0.5,
            liquidation_penalty_scale = 1/40
        ):
        
        super(agentDLVEnv, self).__init__()
    ...
    def step(self, action):
        ...
        if action == 0:
            ...
            nect_target = (self.target_leverage - 1)*(self.collateral_value - self.volatile_part_value)
            action_size = nect_target - self.nect
        ...
        self.LP_margin = self.LP/self.starting_LP
        if action_penalty!=0:
            action_penalty = action_penalty + 50*cost/current_nav
        ...
        if self.cr_margin <= 0:
            self.num_of_liquidations += 1
            liquidation_penalty = liquidation_penalty + deltaFunctionBarier(0)
            if self.is_training:
                terminated = False
                truncated = self.step_count >= self.max_steps - 1
            else:
        ...
        leverage_reward = 0.25*plateau(self.leverage_margin) + 0.75*plateauFlat(self.leverage_margin) - 0.01*abs(self.leverage_margin)
