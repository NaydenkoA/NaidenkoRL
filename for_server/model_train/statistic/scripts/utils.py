...
def plot_episode_history_v2(history, labels, top_leverage, bottom_leverage):
    n = len(history)

    fig, axs = plt.subplots(5, 1, figsize=(12, 10), sharex=True)

    for i in range(n):
        axs[0].plot(history[i]['leverage'], label=labels[i])
    ...
    for i in range(n):
        axs[1].plot(history[i]['loss on spread'], label=labels[i])
    axs[1].set_ylabel('relative loss on spread')

    for i in range(n):
        axs[2].plot(history[i]['boundary leverage'], label=labels[i])
    ...
    for i in range(n):
        axs[3].plot(history[i]['collaterization ratio'], label=labels[i])
    axs[3].axhline(1.2, color='r', linestyle='--', label='min cr')
    axs[3].set_ylabel('collaterization ratio')
    axs[3].legend()

    for i in range(n):
        axs[4].plot(history[i]['reward'], label=labels[i])
