...
class agentDLVEnvV2(gym.Env):
    def __init__(
        ...
        self.pure_timestamp = timestamps.copy()

        self.vol_token_pool = uniswap_v2_sim(
            pool_configs['initial_token0'],
            pool_configs['initial_token1'],
            pool_configs['initial_total_supply'],
            pool_configs['decimals0'],
            pool_configs['decimals1'],
            vol_token_honey_pool_history
        )
    ...
    def reset(self, seed = None, options = None, top_leverage = None, bottom_leverage = None):
        ...
        if self.is_training:
            ...
            self.bottom_leverage = self.bottom_leverage*random.uniform(0.93, 1.07)
            self.top_leverage = self.bottom_leverage + lenght*random.uniform(0.8, 1.2)
            if self.top_leverage >= 3.95:
                self.top_leverage = 3.95
                self.bottom_leverage = 3.95 - lenght*random.uniform(0.8, 1.2)
        ...
        if self.is_mixed:
            ...
            self.timestamp = self.pure_timestamp[N:N + self.mixed_episode_lenght]
        else:
            self.max_episodes = self.pure_max_episodes
            self.nav = self.pure_nav
            self.vol_part = self.pure_vol_part
            self.timestamp = self.pure_timestamp
        ...
        self.current_nect_honey_price = 1
        self.current_timestamp = self.timestamp[0]
        self.current_vol_part = self.vol_part[0]
        self.current_calm_part = 1 - self.vol_part[0]

        self.vol_token_pool.set_history_timestamp(self.current_timestamp)
        self.current_vol_token_honey_price = self.vol_token_pool.get_price('volatile')
        ...
        self.leverage_mean_normed = (self.leverage - self.bottom_leverage)/self.norm
    ...
    def step(self, action):
        ...
        self.current_nect_honey_price = 1
        self.current_timestamp = self.timestamp[self.step_count]
        ...
        leverage_maintain_reward = 2*plateauFlat(self.leverage_normed - 0.5, 0.5) - 1 + 0.2*(min(self.leverage_normed, 0) + min(1 - self.leverage_normed, 0))
        ...
        if done and not self.is_training:
            ...
            info['0 action'] = self.action_counter[0]/self.max_episodes
            info['0.5 rebalancing'] = self.action_counter[1]/self.max_episodes
            info['0.75 rebalancing'] = self.action_counter[2]/self.max_episodes
            info['0.25 rebalancing'] = self.action_counter[3]/self.max_episodes
    ...
    def _update_history(self):
        ...
        self.history_metric['boundary leverage'].append(self.boundary_leverage_normed)
