...
def generateSeveralEpisodesParallel(model_state, configs, n_envs=6, epsilon=None, confidence_level=0.99):
    """
    Run multiple episodes in parallel using threads (safe on Linux servers).

    Args:
        model_state: state_dict of the trained model
        configs: list of environment configs
        n_envs: number of parallel episodes
        epsilon: optional exploration parameter
        confidence_level: optional threshold for action 0
    Returns:
    ...
    with ThreadPoolExecutor(max_workers=n_envs) as executor:
        ...
        for i in range(n_envs):
            ...
            config = configs[i] if i < n_configs else configs[-1]
