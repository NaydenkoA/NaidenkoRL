...
def simulate_data(n_steps, S0 = [85000, 30, 3.5e6, 0.1], rng = None, sigma_token = None, sigma_amounts = None):
    ...
    file_path = os.path.join(script_dir, "wbtc_island_var_model.pkl")
    with open(file_path, "rb") as f:
        var_results = load_pickle(f)
...
class data_generator:
    def __init__(
        ...
        if sigma_token is None:
            self.sigma1 = SIGMA["S1"]
        else:
            self.sigma1 = sigma_token

        if sigma_amounts is None:
            self.sigma2 = SIGMA["S2"]
            self.sigma3 = SIGMA["S3"]
        else:
            self.sigma2 = sigma_amounts
            self.sigma3 = sigma_amounts

        self.sigma4 = SIGMA["S4"]
        self.garch = GARCH

        script_dir = os.path.dirname(__file__)          
        file_path = os.path.join(script_dir, "wbtc_island_var_model.pkl")
        with open(file_path, "rb") as f:
            self.var_results = load_pickle(f)

    def generateSyntheticEpisodeFull(self):
        df = simulate_var_garch(
            self.var_results,
            self.garch,
            self.n_steps, 
        ...
        p = self.initials[0]*np.exp(self.sigma1*np.cumsum(s1_normed))
        v = self.initials[1]*np.exp(self.sigma2*np.cumsum(s2_normed))
        s = self.initials[2]*np.exp(self.sigma3*np.cumsum(s3_normed))
        ts = self.initials[3]*np.exp(self.sigma4*np.cumsum(s4_normed))
    ...
    def generateSyntheticPrice(self):
        df = simulate_var_garch(
            self.var_results,
            self.garch,
            self.n_steps, 
