...
class agentDLVEnvV1(gym.Env):
    def __init__(
        ...
        self.volatile_part = volatile_part.copy() # % of the volatile token (wbtc) in the LP token
    ...
    def step(self, action):
        ...
        leverage = (self.nect + self.cumulative_debt*(1 - self.volatile_part[self.step_count]))/(self.collateral_value - self.volatile_part_value + 1e-10) + 1
        ...
        bound_maintain_reward = plateauFlat(self.leverage_margin, width=0.1*self.target_leverage) - 0.1*abs(self.leverage_margin)
